...
class ReportNS extends ZFrame
{
    ...
    public function test()
    {
        $shopOrder = ShopOrder::find()
            ->asArray()
            ->all();
        $res = $this->getDateDeliver($shopOrder);
        vd($res);
    }
...
//        vd($shop_order);
/*
        $status_logistics = ZArrayHelper::getColumn($shop_order, 'status_logistics');
...
//        foreach ($names as $name)
    ...
    public function getDateDeliver(array $shop_order)
    {
        $shop_shipment_id = ZArrayHelper::getValue($shop_order, 'shop_shipment_id');
        if ($shop_shipment_id === null) return null;

        $date_deliver = $this->shop_shipment
